...
public class GaussXYDeal {
    //  由高斯投影坐标反算成经纬度
    public static double[] GaussToBL(double Y, double X,double midlongitude)//, double *longitude, double *latitude)
    {
        ...
        a=6378137.0; f=1/298.257223563; //wgs84坐标系参数
        ZoneWide = 6; ////6度带宽
       // ProjNo = (int)(X/1000000L) ; //查找带号
        //longitude0 = (ProjNo-1) * ZoneWide + ZoneWide / 2;
        longitude0= midlongitude;
       ...
       // X0 = ProjNo*1000000L+500000L;
        ...
        //转换为度 DD
        output[0] = latitude1 / iPI;
        output[1] = longitude1 / iPI;
        return output;
        //*longitude = longitude1 / iPI;
        //*latitude = latitude1 / iPI;
    }
    ////  由经纬度反算成高斯投影坐标
    public static double[] GaussToBLToGauss(double latitude ,double longitude,double midlongitude) {
        int ProjNo = 0;
        int ZoneWide; // //带宽
        double[] output = new double[2];
    ...
    public static double[] DifXy(double latitude ,double longitude,double rx,double ry,double midlatitude){
        double[] output = new double[2];
        output=GaussToBLToGauss(latitude,longitude,midlatitude);
        double x=output[0];
        double y=output[1];
        output[0]=x-rx;
        output[1]=y-ry;
