...
public class KmlTrack extends KmlGeometry {
	...
	@Override public void buildOverlay(Files files, Styler styler, KmlPlacemark kmlPlacemark,
		...
		List<GeoPoint> coordinateList=mCoordinates;
		data.createPolyline(files,sqlPolyline);
		for(int k=0;k<coordinateList.size();k++){
			GeoPoint point= Location3TheConvert.ConverToGCJ2(coordinateList.get(k).getLatitude(),coordinateList.get(k).getLongitude(), LatStyle.GPSSYTELE);
			Sqlpoint sqlpoint=new Sqlpoint();
			sqlpoint .setLatitude(point.getLatitude());
			sqlpoint.setLongitude(point.getLongitude());
			data.createPoint(sqlPolyline,sqlpoint);
		}
