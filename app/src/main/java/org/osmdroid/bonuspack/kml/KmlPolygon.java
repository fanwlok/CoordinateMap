...
public class KmlPolygon extends KmlGeometry {
	...
	@Override public void buildOverlay(Files files,  Styler styler, KmlPlacemark kmlPlacemark,
		...
		SqlPolyline msqlPolyline=new SqlPolyline();
		msqlPolyline.setName(kmlPlacemark.mName);
		msqlPolyline.setDescribe(kmlPlacemark.mDescription);
		String points= StringToPoint.getGeoPoints(mCoordinates,LatStyle.GPSSYTELE);
		msqlPolyline.setPoints(points);
		data.createPolyline(files,msqlPolyline);
		if (mHoles != null){
		for(int i=0;i<mHoles.size();i++){
			...
			String pointlist= StringToPoint.getGeoPoints(mHoles.get(i),LatStyle.GPSSYTELE);
			sqlPolyline.setPoints(pointlist);
